...
const size = runners.filter(function(item) {
...
runners.map(function(value) {
  donate.push(value.donation);
});

const dono = donate.reduce(function(total, amount) {
  return total + amount;
});

ticketPriceTotal.push(dono);
console.log(donate);
...
// Problem 1 - The director needs each contestants first name, last name, and email in its own list.

let nameEmail = [];
const eName = runners.map(function(value) {
  return value.first_name + " " + value.last_name + " " + value.email;
});

nameEmail.push(eName);
console.log(nameEmail);

// Problem 2 - The director wants a list of all the people who donated 150 or more.
let donoReward = [];
const reward = runners.filter(function(value) {
  return value.donation > 149;
});

donoReward.push(reward);
console.log(donoReward);

// Problem 3 - The director wants a list of all the shirt sizes only.
let sizeOfShirts = [];

runners.forEach(function(value) {
  sizeOfShirts.push(value.shirt_size);
});

console.log(sizeOfShirts);
